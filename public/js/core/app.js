...
function NXEstimates() {
    ...
    $("#commonModalForm").validate({
        rules: {
            ...
            expense_pay_to: "required",
            expense_description: "required",
            expense_amount: "required",
            expense_authorized_by: "required",
...
function NXProformaInvoiceCreate() {
    //validation
    $("#commonModalForm").validate().destroy();
    $("#commonModalForm").validate({
        ignore: ":hidden:not(.mysql-date)", // Ignore hidden fields except .mysql-date
        rules: {
            proforma_invoice_clientid: "required",
            proforma_invoice_quotation: "required",
            proforma_invoice_workorder: "required",
            proforma_invoice_project: "required",
            proforma_invoice_date: "required",
            proforma_invoice_due_date: "required",
        },
        submitHandler: function (form) {
            //ajax form, so initiate ajax request here
            nxAjaxUxRequest($("#commonModalSubmitButton"));
...
function NXSettingsDeliverables() {
    var $actions = $("#list-page-actions");
    //replace action buttons
    $(".parent-page-actions").html('');
    $actions.prependTo(".parent-page-actions");
...
function NXSettingsDeliverablesCreate() {
    $("#commonModalForm").validate().destroy();
    $("#commonModalForm").validate({
        rules: {
...
function NXSettingsTechStackCreate() {
    $("#commonModalForm").validate().destroy();
    $("#commonModalForm").validate({
        rules: {
            techstack_title: "required",
        },
        submitHandler: function (form) {
            nxAjaxUxRequest($("#commonModalSubmitButton"));
        }
...
function NXSettingsProjectModules() {
    var $actions = $("#list-page-actions");
    //replace action buttons
    $(".parent-page-actions").html('');
    $actions.prependTo(".parent-page-actions");
...
function NXSettingsProjectModuleCreate() {
    $("#commonModalForm").validate().destroy();
    $("#commonModalForm").validate({
        rules: {
            project_module_title: "required",
            project_module_deliverable: "required",
            estimated_time_intern_hours: "required",
            estimated_time_intern_minutes: "required",
            estimated_time_junior_hours: "required",
            estimated_time_junior_minutes: "required",
            estimated_time_senior_hours: "required",
            estimated_time_senior_minutes: "required",
        },
        submitHandler: function (form) {
            nxAjaxUxRequest($("#commonModalSubmitButton"));
        }
...
function NXremindersDatePicker() {
    ...
    // Retrieve preset date or use current date as fallback
    var preset_date = $("#reminders-datetimepicker").attr('data-preset-date') || moment().format('YYYY-MM-DD HH:mm');
    // Initialize the datetime picker
    $('#reminders-datetimepicker').datetimepicker({
        format: 'YYYY-MM-DD HH:mm',
        defaultDate: preset_date, // Set the initial date
        inline: true,             // Keep the picker always visible
        sideBySide: true          // Display calendar and time selector side-by-side
    });

    // Update hidden input field on date/time change
...
function NXcloseSidePanel($self) {
    ...
    if (target) {
        ...
        if ($("#sidepanel-filter-workorder").length) {
            $("#sidepanel-filter-workorder").find('select').val(null).trigger('change').empty();
            var filterQuotationDropdown = target.find("#filter_workorder_quotation");
            if (filterQuotationDropdown.length > 0) {
                filterQuotationDropdown.prop("disabled", true);
            }
        } else if ($('#sidepanel-filter-proforma-invoices').length) {
            target.find('select').val(null).trigger('change');
            var filterProformaProjectDropdown = target.find("#proforma_filter_bill_projectid");
            if (filterProformaProjectDropdown.length > 0) {
                filterProformaProjectDropdown.val(null).trigger('change').empty();
                filterProformaProjectDropdown.prop("disabled", true);
            }
        } else if ($('#sidepanel-filter-invoices').length) {
            target.find('select').val(null).trigger('change');
            var filterInvoiceProjectDropdown = target.find("#filter_bill_projectid");
            if (filterInvoiceProjectDropdown.length > 0) {
                filterInvoiceProjectDropdown.val(null).trigger('change').empty();
                filterInvoiceProjectDropdown.prop("disabled", true);
            }
        } else {
            console.log('othres')
            //reset input fields
...
function NXAddEditWorkorder() {
    // Initialize Dropzone
    var myDropzone = new Dropzone(".import-workorder-files-upload", {
        url: "/workorder/file/upload",
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        maxFiles: 1,
        ...
        thumbnailHeight: null,
        init: function () {
            var dz = this; // Dropzone instance reference
            ...
            this.on("error", function (file, message, xhr) {
                ...
                dz.removeFile(file); // Remove file on error
            });

            $(".remove-file-btn").off("click").on("click", function () {
                var uploadedFiles = dz.getAcceptedFiles(); // Get uploaded files
                if (uploadedFiles.length > 0) {
                    dz.removeFile(uploadedFiles[0]); // Remove only the first file (since maxFiles = 1)
                }
                let button = $(this); // Store reference
                let fileUrl = button.data('file-url');
                let workorderId = button.data('workorder-id');

                // Send AJAX request to remove file from backend
                $.ajax({
                    url: NX.site_url + '/workorder/file/remove',
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    data: {
                        'importing_file_name': $("#importing-file-name").val() ?? '',
                        'importing_file_uniqueid': $("#importing-file-uniqueid").val() ?? '',
                        'file_url': fileUrl ?? '',
                        'workorder_id': workorderId ?? ''
                    }
                }).done(function (response) {
                    if (fileUrl && workorderId) {
                        button.data('file-url', '').removeAttr("data-file-url");
                        button.data('workorder-id', '').removeAttr("data-workorder-id");
                        $('.workorder-file-attachment a').removeAttr("href download target");
                    }
                    $(".x-file-attachment").fadeOut();
                    $(".uploaded-file-name").text("");
                    $("#importing-file-name").val("");
                    $("#importing-file-uniqueid").val("");
                    $("#workorder-file-attachments-wrapper").fadeIn();
                }).fail(function (jqXHR) {
                    // Handle error response
                    console.error("Error removing file:", jqXHR.responseText);
                });
            });
        },
        success: function (file, response) {
            ...
            $("#importing-file-name").val(response.filename);
            $("#importing-file-uniqueid").val(response.uniqueid);
            $("#workorder-file-attachments-dropzone-wrapper, #workorder-file-attachments-wrapper").fadeOut();
            $(".x-file-attachment").fadeIn();
            $(".uploaded-file-name").text(response.filename);
        }
    });



    // Destroy previous validation instance
    if ($.fn.validate) {
        $("#commonModalForm").validate().destroy();
    }
    ...
    $("#commonModalForm").validate({
        ...
        submitHandler: function (form) {
            $("#workorder_quotation").prop("disabled", false); // Enable field before submit
            nxAjaxUxRequest($("#commonModalSubmitButton"));
        },
    });
}
