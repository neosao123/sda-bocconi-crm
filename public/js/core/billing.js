...
NXINVOICE.DOM.domState = function () {
    NXINVOICE.log(
        "[billing] state() - setting billing dom state- [payload]",
...
NXINVOICE.DOM.itemNewLine = function (data = {}) {
    ...
    var item_hsncode = data.item_hsncode != null ? data.item_hsncode : "";
    ...
    var item_taxable_amount = data.item_taxable_amount != null ? data.item_taxable_amount : "";
    ...
    var item_tax_status =
        data.item_tax_status != null ? data.item_tax_status : "";
    var item_id = data.item_id != null ? data.item_id : "";
    var has_estimation_notes =
        data.has_estimation_notes != null ? data.has_estimation_notes : "no";
    var estimation_notes =
        data.estimation_notes != null ? data.estimation_notes : "";
    ...
    lineitem.find(".js_item_hsncode").val(item_hsncode);
    ...
    lineitem.find(".js_item_taxable_amount").val(item_taxable_amount);
    ...
    lineitem
        .find(".js_item_hsncode")
        .attr("name", "js_item_hsncode[" + uniqueid + "]");
    lineitem
    ...
    lineitem
        .find(".js_item_taxable_amount")
        .attr("name", "js_item_taxable_amount[" + uniqueid + "]");
    lineitem
    ...
    lineitem
        .find(".js_item_tax_status")
        .attr("name", "js_item_tax_status[" + uniqueid + "]");
    ...
    if (has_estimation_notes == "yes") {
        ...
        if (
            $("#billing-estimation-notes-template").find(".estimation-notes-template")
                .length > 0
        ) {
            var lineitem_estimation_notes = $("#billing-estimation-notes-template")
                .find(".estimation-notes-template")
                .clone();
            lineitem_estimation_notes
                .find(".estimation-notes-text")
                .html(HTMLEntitiesDecode(estimation_notes));
            lineitem_estimation_notes
                .find(".estimation-notes-text")
                .html(HTMLEntitiesDecode(estimation_notes));
...
NXINVOICE.DOM.dimensionsNewLine = function (data = {}) {
    NXINVOICE.log(
        "[billing] NXINVOICE.DOM.dimensionsNewLine() - cloning dimensions line - [payload]"
    );
    NXINVOICE.log(data);
    ...
    var item_quantity = data.item_quantity != null ? data.item_quantity : "";
    var item_unit =
        data.item_unit != null ? data.item_unit : NXLANG.dimensions_default_unit;
    ...
    //new element (plain or time)
    var lineitem = $("#billing-line-template-dimensions")
        .find("tr")
        .first()
        .clone();
    ...
    lineitem
        .find(".js_item_length")
        .attr("name", "js_item_length[" + uniqueid + "]");
    lineitem
        .find(".js_item_width")
        .attr("name", "js_item_width[" + uniqueid + "]");
    lineitem.find(".js_item_unit").attr("name", "js_item_unit[" + uniqueid + "]");
    lineitem.find(".js_item_rate").attr("name", "js_item_rate[" + uniqueid + "]");
    lineitem
        .find(".js_item_total")
...
$(document).ready(function () {
    console.log("called recalculateLines()");
...
NXINVOICE.CALC.recalculateLines = function () {
    ...
    $("#billing-items-container")
        ...
        .each(function () {
            ...
            var taxable_amount = lineitem.find(".js_item_taxable_amount");
            ...
            var hsncode = lineitem.find(".js_item_hsncode").val();
            ...
            if (type == "plain") {
                ...
                if (quantity > 0 && rate > 0) {
                    ...
                    NXINVOICE.log(
                        "[billing] recalculateLines() - line item is valid. [bill_tax_type]: " +
                        $("#bill_tax_type").val()
                    );
                    console.log("recalculateLines() ", $("#bill_tax_type").val());
                    if ($("#bill_tax_type").val() === 'summary') {
                        total.val(nxFormatDecimal(Number(linetotal).toFixed(2)));
                    } else if ($("#bill_tax_type").val() === 'inline') {
                        taxable_amount.val(nxFormatDecimal(Number(linetotal).toFixed(2)));
                    }

                    //increase bill total
                    NXINVOICE.DATA.calc_total += linetotal;
                    NXINVOICE.log(
                        "[billing] recalculateLines() - line item is valid. [line item total]: " +
                        linetotal
                    );
                } else {
                    NXINVOICE.log(
                        "[billing] recalculateLines() - line item is invalid and is skipped"
                    );
                    taxable_amount.val("");
                    total.val("");
                }

                NXINVOICE.CALC.recalculateLineTaxes();
            }
...
NXINVOICE.CALC.calculateTotalLineItem = function () {
    ...
    $("#billing-items-container")
        ...
        .each(function () {
            var lineitem = $(this);
            total_amount += Number(lineitem.find(".js_item_taxable_amount").val());
        });

    NXINVOICE.log(
        "[billing] recalculateLineTaxes() - total_amount: " + total_amount
    );
    return total_amount;
}
...
NXINVOICE.CALC.recalculateLineTaxes = function () {
    NXINVOICE.log(
        "[billing] recalculateLineTaxes() - validating and recalculating each line tax"
    );

    var totalAmountWithoutTax = NXINVOICE.CALC.calculateTotalLineItem();
    NXINVOICE.log(
        "[billing] recalculateLineTaxes() - totalAmountWithoutTax: " + totalAmountWithoutTax
    );
    // reset the total bill tax
    var total_tax = Number(0);
    var total_tax_rate = Number(0);
    ...
    $("#billing-items-container")
        ...
        .each(function () {
            NXINVOICE.log(
                "[billing] recalculateLineTaxes() - found a line item. Now validating and calculating its tax"
            );

            var lineitem = $(this);
            var line_tax = 0;
            var line_discount = 0;
            var line_taxable_amount = lineitem.find(".js_item_taxable_amount").val();
            var line_tax_data = lineitem.find(".js_item_tax").val();
            console.log("line_tax_data: ", line_tax_data);
            var line_tax_data = line_tax_data.split("|");
            var line_tax_rate = line_tax_data[0];
            var line_total = lineitem.find(".js_item_total");
            ...
            var bill_tax_type = $("#bill_tax_type").val();


            NXINVOICE.log(
                line_total +
                "[billing] recalculateLineTaxes() - discount type -" + bill_discount_type + ' percentage:  ' + bill_discount_percentage + ' amount:  ' + bill_discount_amount
            );

            //   var line_tax_name = line_tax_data[1];
            console.log("NXINVOICE.is_numeric(line_tax_rate): ", NXINVOICE.is_numeric(line_tax_rate))
            ...
            if (NXINVOICE.is_numeric(line_tax_rate)) {
                ...
                if (bill_discount_type == "percentage") {
                    ...
                    if (
                        ...
                        Number($("#js_bill_discount_percentage").val()) <= 0
                    ) {
                        NXINVOICE.log(
                            "[billing] recalculateLineTaxes() - the bill discount type is percentage"
                        );
                        line_discount = 0;
                    }
                }
                ...
                if (bill_discount_type == "fixed") {
                    ...
                    if (Number($("#js_bill_discount_amount").val()) <= 0) {
                        NXINVOICE.log(
                            "[billing] recalculateLineTaxes() - the bill discount type is fixed"
                        );
                        line_discount = 0;
                    }
                }
                ...
                if (bill_discount_type === 'fixed') {
                    ...
                    line_discount = (line_taxable_amount / totalAmountWithoutTax) * bill_discount_amount;
                } else if (bill_discount_type === 'percentage') {
                    NXINVOICE.log(
                        "[billing] recalculateLineTaxes() - the bill discount type is percentage"
                    );
                    var cal_bill_discount_amount = (totalAmountWithoutTax * bill_discount_percentage) / 100
                    line_discount = (line_taxable_amount / totalAmountWithoutTax) * cal_bill_discount_amount;
                } else if (bill_discount_type === 'none') {
                    NXINVOICE.log(
                        "[billing] recalculateLineTaxes() - the bill discount type is none"
                    );
                    line_discount = 0;
                }
                console.log("bill_tax_type: recalculateLineTaxes() ", bill_tax_type);
                if (bill_tax_type === 'summary') {

                    return;
                    //validation - do we have a valid tax rate
                    if (line_tax_rate == 0) {
                        console.log("line_tax_rate == 0", line_tax_rate, line_tax_rate == 0);
                        NXINVOICE.log(
                            "[billing] recalculateLineTaxes() - the tax is zero rate. will skip this item"
                        );
                        //skip this line
                        return;
                    }
                    ...
                    if (line_total.val() == 0) {
                        NXINVOICE.log(
                            "[billing] recalculateLineTaxes() - the line item total is zero. will skip this item"
                        );
                        //skip this line
                        return;
                    }
                    ...
                    //calculate the tax
                    line_tax = (Number(line_total.val()) * line_tax_rate) / 100;
                    console.log("line_tax: ", line_tax, "line_total: ", line_total.val());
                    NXINVOICE.log(
                        "[billing] recalculateLineTaxes() - tax has been calculated for this line (" +
                        line_tax +
                        ")"
                    );
                    NXINVOICE.log(
                        "[billing] recalculateLineTaxes() - line tax rate (" +
                        line_tax_rate +
                        ")"
                    );
                    line_total.val(nxFormatDecimal(Number(line_total.val()) + Number(line_tax)));
                }


                if (bill_tax_type === 'inline') {
                    //validation - do we have a valid tax rate
                    if (line_tax_rate == 0) {
                        NXINVOICE.log(
                            "[billing] recalculateLineTaxes() - the tax is zero rate. will skip this item"
                        );
                        line_total.val(nxFormatDecimal(Number(line_taxable_amount) - Number(line_discount)));
                        //skip this line
                        return;
                    }
                    //validation - do we have a valid line total
                    if (line_taxable_amount == 0) {
                        NXINVOICE.log(
                            "[billing] recalculateLineTaxes() - the line item total is zero. will skip this item"
                        );
                        //skip this line
                    ...
                    //calculate the tax
                    line_tax = ((Number(line_taxable_amount) - Number(line_discount)) * line_tax_rate) / 100;
                    NXINVOICE.log(
                        "[billing] recalculateLineTaxes() - tax has been calculated for this line (" +
                        line_tax +
                        ")"
                    ...
                    line_total.val(nxFormatDecimal((Number(line_taxable_amount) - Number(line_discount)) + Number(line_tax)));
                }
                ...
                total_tax_rate += Number(line_tax_rate);
                total_tax += Number(line_tax);
                NXINVOICE.log(
                    "[billing] recalculateLineTaxes() - running total tax is now (" +
                    total_tax +
                    ")"
                );
            }
    ...
    return {
        ...
        total_tax_rate: Number(total_tax_rate).toFixed(2),
    };
};
...
NXINVOICE.DOM.addSelectedProductItems = function (self) {
    ...
    $("#items-list-table, #category-items-list-table")
        .find(".items-checkbox")
        .each(function () {
            if ($(this).is(":checked")) {
                //save to object
                ...
                if ($(this).attr("data-type") == "standard") {
                    NXINVOICE.log(
                        "[billing] NXINVOICE.DOM.addSelectedProductItems() - selected item in s [standard item]. Playload: "
                    );
                    NXINVOICE.log(data);
                ...
                if ($(this).attr("data-type") == "dimensions") {
                    NXINVOICE.log(
                        "[billing] NXINVOICE.DOM.addSelectedProductItems() - selected item in s [dimensions item]. Playload: "
                    );
                    NXINVOICE.log(data);
...
NXINVOICE.CALC.reclaculateBill = function (self) {
    ...
    var bill_tax_type_define = $("#bill_tax_type_define").val();

    //recalculate lines
    NXINVOICE.CALC.recalculateLines();

    NXINVOICE.log("TAX TYPE: " + bill_tax_type_define);
    ...
    $("#billing-items-container")
        ...
        .each(function () {
            ...
            if ($("#bill_tax_type").val() == "summary") {
                ...
                //each line item total
                var linetotal = Number(lineitem.find(".js_item_taxable_amount").val());
                //validate that its a number
                if (typeof linetotal == "number") {
                    NXINVOICE.log(
                        "[billing] reclaculateBill() - valid line item found [total]: (" +
    ...
    if (bill_discount_amount > 0) {
        var bill_amount_before_tax = nxFormatDecimal(bill_subtotal) - nxFormatDecimal(bill_discount_amount);
        //show subtotal
        ...
        bill_amount_before_tax = nxFormatDecimal(bill_subtotal);
        //log
        NXINVOICE.log(
    ...
    if ($("#bill_tax_type").val() == "summary") {
        ...
        $("#bill_logic_taxes")
            ...
            .each(function () {
                ...
                //create table row
                if (bill_tax_type_define === "CSGST") {
                    tax_row +=
                        '<tr class="billing-sums-tax-container" id="billing-sums-tax-container-c-' +
                        uniqueid +
                        '">' +
                        "<td>" +
                        "C-GST" +
                        ...
                        "%)</span></td>" +
                        "<td>" +
                        accounting.formatMoney(tax_amount / 2) +
                        "</td></tr>";

                    tax_row +=
                        '<tr class="billing-sums-tax-container" id="billing-sums-tax-container-s-' +
                        uniqueid +
                        '">' +
                        "<td>" +
                        "S-GST" +
                        ...
                        accounting.formatMoney(tax_amount / 2) +
                        "</td></tr>";
                } else if (bill_tax_type_define === "IGST") {
                    tax_row +=
                        '<tr class="billing-sums-tax-container" id="billing-sums-tax-container-i-' +
                        uniqueid +
                        '">' +
                        "<td>" +
                        "I-GST" +
    ...
    if ($("#bill_tax_type").val() == "inline") {
        ...
        console.log(
            "foo: " + total_inline_tax.total_tax,
            total_inline_tax.total_tax_rate
        );

        let line_total_tax_rate = total_inline_tax.total_tax_rate;
        let line_total_tax_amount = total_inline_tax.total_tax;

        //finalise tax for this bill
        if (
            NXINVOICE.is_numeric(line_total_tax_amount) &&
            ...
            //log
            NXINVOICE.log(
                "[billing] reclaculateBill() - this bill has [line] based tax totalling: (" +
            ...
            //     '<td>' + NXLANG.tax + '</td>' +
            //     '<td>' + accounting.formatMoney(line_total_tax_amount) + '</td></tr>';

            // if (bill_tax_type_define === "CSGST") {
            //     tax_row +=
            //         '<tr class="billing-sums-tax-container" id="billing-sums-tax-container-c-inline">' +
            //         "<td>" +
            //         "C-GST" +
            //         ' <span class="x-small">(' +
            //         line_total_tax_rate / 2 +
            //         "%)</span></td>" +
            //         "<td>" +
            //         accounting.formatMoney(line_total_tax_amount / 2) +
            //         "</td></tr>";

            //     tax_row +=
            //         '<tr class="billing-sums-tax-container" id="billing-sums-tax-container-s-inline">' +
            //         "<td>" +
            //         "S-GST" +
            //         ' <span class="x-small">(' +
            //         line_total_tax_rate / 2 +
            ...
            // } else if (bill_tax_type_define === "IGST") {
            //     tax_row +=
            //         '<tr class="billing-sums-tax-container" id="billing-sums-tax-container-i-inline">' +
            //         "<td>" +
            //         "I-GST" +
            //         ' <span class="x-small">(' +
            //         line_total_tax_rate +
            //         "%)</span></td>" +
            //         "<td>" +
            //         accounting.formatMoney(line_total_tax_amount) +
            //         "</td></tr>";
            // } else {
            tax_row +=
                '<tr class="billing-sums-tax-container" id="billing-sums-tax-container-inline">' +
                "<td>" +
                'Tax' +
                '</td>' +
                "<td>" +
            ...
            bill_tax_total_amount = line_total_tax_amount;
    ...
    NXINVOICE.log(
        "[billing] reclaculateBill() -  bill sub_total:  (" +
        bill_subtotal + ") bill discount amount: (" + Number(bill_discount_amount).toFixed(2) + ") Bill tax tota amount: (" + Number(bill_tax_total_amount) +
        ")");

    //update bills final sums
    bill_final_amount =
        bill_subtotal - Number(bill_discount_amount).toFixed(2) + Number(bill_tax_total_amount);
    NXINVOICE.log(
...
NXINVOICE.CALC.validateLines = function (self) {
    ...
    $("#billing-items-container")
        ...
        .each(function () {
            ...
            //each input fields
            var $hsncode = lineitem.find(".js_item_hsncode");
            var $description = lineitem.find(".js_item_description");
