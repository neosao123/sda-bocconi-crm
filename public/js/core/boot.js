...
function nxTinyMCEBasic(tinyMCEHeight = 300, tinyMCESelector = '.tinymce-textarea') {
    ...
    tinymce.init({
        selector: tinyMCESelector,
        content_style: '.mce-content-body strong, .mce-content-body b { font-weight: bold !important; }',
        language: NX.system_language,
        mode: 'exact',
        theme: "modern",
        skin: 'light',
        ...
        document_base_url: NX.site_url,
        toolbar: 'fontsizeselect bold italic link bullist numlist image media alignleft aligncenter alignright outdent indent hr table code fullscreen forecolor backcolor',
        plugins: [
            "fullscreen", "image", "paste", "link", "code", "media", "autoresize", "codesample",
            "table", "hr", "pagebreak", "toc", "advlist", "lists", "textpattern", "colorpicker"
        ],
        height: tinyMCEHeight,
        setup: function (editor) {
            editor.on('change', function () {
...
function nxTinyMCEBasicDescription(tinyMCEHeight = 300, tinyMCESelector = '.tinymce-textarea-description') {
    ...
    tinymce.init({
        ...
        menubar: false,
        statusbar: false,
        // ❌ remove forced_root_block: false
        forced_root_block: 'p', // keep default for proper nesting
        autoresize_min_height: tinyMCEHeight,
        document_base_url: NX.site_url,
        toolbar: 'formatselect fontsizeselect bold italic link bullist numlist outdent indent image media alignleft aligncenter alignright hr table code fullscreen removeformat',
        plugins: [
            "fullscreen", "image", "paste", "link", "code", "media", "autoresize", "codesample",
...
function nxTinyMCEDocuments(tinyMCEHeight = 800, tinyMCESelector = '.tinymce-document-textarea') {
    ...
    tinymce.init({
        ...
        content_css: [
            ...
            "/public/vendor/fonts/all-icons/styles.css",
...
NX.resetFilterPanelFields = function ($self) {
    ...
    // Check if filter_workorder_quotation exists before resetting
    var filterQuotationDropdown = target.find("#filter_workorder_quotaticlientQuotationWorkorderToggleon");
    if (filterQuotationDropdown.length > 0) {
        filterQuotationDropdown.val(null).trigger('change').empty(); // Remove all options
        filterQuotationDropdown.prop("disabled", true); // Disable the dropdown
    }
...
NX.clientAndQuotationClearToggle = function ($self) {

    //the quotation dropdown list
    var quotation_dropdown = $("#" + $self.attr('data-quotation-dropdown'));

    //cear & disable quotation dropdown
    quotation_dropdown.prop("disabled", true);
    quotation_dropdown.empty().trigger("change");
}

NX.clientAndQuotationToggle = function (e, $self) {
    //the client's id
    var client_id = e.params.data.id;
    ...
    //add loading class to quotation dropdown
    $(".dynamic_" + $self.attr('data-quotation-dropdown')).addClass('loading');

    //disable or main dropdown
    $self.prop("disabled", true);

    //backend ajax call to get clients quotation
    $.ajax({
        type: 'GET',
        url: NX.site_url + "/feed/quotation?ref=" + feed_ref + "&client_id=" + client_id
    }).then(function (data) {

        //loop through the returned array and create new select option items
        if (data.length > 0) {
            var option = '';
            quotation_dropdown.append(option).trigger('change');
        }
        for (var i = 0; i <= data.length - 1; i++) {
            var option = new Option(data[i].value, data[i].id, false, false);
            quotation_dropdown.append(option).trigger('change');
        ...
        //do we have any data
        if (i > 0) {
            quotation_dropdown.prop("disabled", false);

            // manually trigger the `select2:select` event
            quotation_dropdown.trigger({
                type: 'select2:select',
                params: {
                    data: data
                }
        ...
        //remove loading class to quotation dropdown
        $(".dynamic_" + $self.attr('data-quotation-dropdown')).removeClass('loading');
...
NX.filterClientAndQuotationClearToggle = function ($self) {

    //the quotation dropdown list
    var filter_quotation_dropdown = $("#" + $self.attr('data-filter-quotation-dropdown'));

    //cear & disable quotation dropdown
    filter_quotation_dropdown.prop("disabled", true);
    filter_quotation_dropdown.empty().trigger("change");
}

NX.filterClientAndQuotationToggle = function (e, $self) {
    //the client's id
    var client_id = e.params.data.id;
    ...
    //the quotation dropdown list
    var filter_quotation_dropdown = $("#" + $self.attr('data-filter-quotation-dropdown'));
    console.log(filter_quotation_dropdown);
    //cear & disable quotation dropdown
    filter_quotation_dropdown.prop("disabled", true);
    filter_quotation_dropdown.empty().trigger("change");

    //add loading class to quotation dropdown
    $(".dynamic_" + $self.attr('data-filter-quotation-dropdown')).addClass('loading');
    ...
    $.ajax({
        ...
        if (data.length > 0) {
            var option = '';
            filter_quotation_dropdown.append(option).trigger('change');
        }
        for (var i = 0; i <= data.length - 1; i++) {
            var option = new Option(data[i].value, data[i].id, false, false);
            filter_quotation_dropdown.append(option).trigger('change');
        ...
        //do we have any data
        if (i > 0) {
            filter_quotation_dropdown.prop("disabled", false);
            // manually trigger the `select2:select` event
            filter_quotation_dropdown.trigger({
                type: 'select2:select',
                params: {
                    data: data
                }
        ...
        //remove loading class to quotation dropdown
        $(".dynamic_" + $self.attr('data-filter-quotation-dropdown')).removeClass('loading');
...
NX.clientQuotationWorkorderClearToggle = function ($self) {
    var quotation_dropdown = $("#" + $self.attr('data-quotation-dropdown'));
    var workorder_dropdown = $("#" + quotation_dropdown.attr('data-workorder-dropdown'));

    // Clear & disable quotation and workorder dropdowns
    quotation_dropdown.prop("disabled", true).empty().trigger("change");
    workorder_dropdown.prop("disabled", true).empty().trigger("change");
};

NX.clientQuotationWorkorderToggle = function (e, $self) {
    var client_id = e.params.data.id;
    var quotation_feed_ref = $self.attr('data-quotation-feed-request-type');
    var quotation_dropdown = $("#" + $self.attr('data-quotation-dropdown'));
    var workorder_dropdown = $("#" + quotation_dropdown.attr('data-workorder-dropdown'));

    // Clear & disable both dropdowns
    quotation_dropdown.prop("disabled", true).empty().trigger("change");
    workorder_dropdown.prop("disabled", true).empty().trigger("change");
    ...
    $.ajax({
        type: 'GET',
        url: NX.site_url + "/feed/quotation?ref=" + quotation_feed_ref + "&client_id=" + client_id
    }).done(function (data) {
        if (data.length > 0) {
            data.forEach(function (item) {
                quotation_dropdown.append(new Option(item.value, item.id, false, false));
            });
            quotation_dropdown.prop("disabled", false).trigger("change");
            // Select the first quotation automatically
            quotation_dropdown.val(data[0].id).trigger({
                type: 'select2:select',
                params: { data: data[0] }
            });
        }
    }).fail(function () {
        console.log("Quotation Fetch Error");
    }).always(function () {
        $(".dynamic_" + $self.attr('data-quotation-dropdown')).removeClass('loading');
        $self.prop("disabled", false);
    });
...
NX.quotationWorkorderClearToggle = function ($self) {
    var workorder_dropdown = $("#" + $self.attr('data-workorder-dropdown'));
    workorder_dropdown.prop("disabled", true).empty().trigger("change");
};

NX.quotationWorkorderToggle = function (e, $self) {
    var quotation_id = e.params.data.id;
    var client_id = $('#proforma_invoice_clientid').val();
    var workorder_feed_ref = $self.attr('data-workorder-feed-request-type');
    var project_quotation_feed_ref = $self.attr('data-project-quotation-feed-request-type');
    var project_client_feed_ref = $self.attr('data-project-client-feed-request-type');
    var workorder_dropdown = $("#" + $self.attr('data-workorder-dropdown'));
    var project_dropdown = $("#" + $self.attr('data-project-dropdown'));

    // Clear & disable workorder dropdown
    workorder_dropdown.prop("disabled", true).empty().trigger("change");
    project_dropdown.prop('disabled', true).empty().trigger('change');

    // Show loading effect
    $(".dynamic_" + $self.attr('data-workorder-dropdown')).addClass('loading');
    $(".dynamic_" + $self.attr('data-project-dropdown')).addClass('loading');
    $self.prop("disabled", true);

    $.ajax({
        type: 'GET',
        url: NX.site_url + "/feed/projects?ref=" + project_quotation_feed_ref + "&quotation_id=" + quotation_id
    }).done(function (data) {
        console.log(data.length > 0, data);
        if (data.length > 0) {
            data.forEach(function (item) {
                project_dropdown.append(new Option(item.value, item.id, false, false));
            });
            project_dropdown.prop("disabled", false).trigger("change");
            // Select the first quotation automatically
            project_dropdown.val(data[0].id).trigger({
                type: 'select2:select',
                params: { data: data[0] }
            });
        } else {
            $.ajax({
                type: 'GET',
                url: NX.site_url + "/feed/projects?ref=" + project_client_feed_ref + "&client_id=" + client_id
            }).done(function (data) {
                if (data.length > 0) {
                    data.forEach(function (item) {
                        project_dropdown.append(new Option(item.value, item.id, false, false));
                ...
                $(".dynamic_" + $self.attr('data-project-dropdown')).removeClass('loading');
                $self.prop("disabled", false);
            });
        }
    }).fail(function () {
    ...
    // Fetch workorders for the selected quotation
    $.ajax({
        type: 'GET',
        url: NX.site_url + "/feed/workorder?ref=" + workorder_feed_ref + "&quotation_id=" + quotation_id
    }).done(function (workorderData) {
        if (workorderData.length > 0) {
            workorderData.forEach(function (item) {
                workorder_dropdown.append(new Option(item.value, item.id, false, false));
            });

            workorder_dropdown.prop("disabled", false).trigger("change");
        }
    }).fail(function () {
        console.log("Workorder Fetch Error");
    }).always(function () {
        $(".dynamic_" + $self.attr('data-workorder-dropdown')).removeClass('loading');
        $self.prop("disabled", false);
    });
};
...
// ----------------------------- For Invoice --------------------
NX.clientProjectProformaInvoiceClearToggle = function ($self) {
    var project_dropdown = $("#" + $self.attr('data-projects-dropdown'));
    var proformainvoice_dropdown = $("#" + project_dropdown.attr('data-proformainvoice-dropdown'));

    // Clear & disable project and workorder dropdowns
    project_dropdown.prop("disabled", true).empty().trigger("change");
    proformainvoice_dropdown.prop("disabled", true).empty().trigger("change");
};

NX.clientProjectProformaInvoiceToggle = function (e, $self) {
    var client_id = e.params.data.id;
    var client_feed_ref = $self.attr('data-feed-request-type');
    var project_dropdown = $("#" + $self.attr('data-projects-dropdown'));
    var proformainvoice_dropdown = $("#" + project_dropdown.attr('data-proformainvoice-dropdown'));
    ...
    $.ajax({
        type: 'GET',
        url: NX.site_url + "/feed/projects?ref=" + client_feed_ref + "&client_id=" + client_id
    }).done(function (data) {
        if (data.length > 0) {
            data.forEach(function (item) {
                project_dropdown.append(new Option(item.value, item.id, false, false));
...
NX.projectProformaInvoiceClearToggle = function ($self) {
    var proformainvoice_dropdown = $("#" + $self.attr('data-proformainvoice-dropdown'));
    proformainvoice_dropdown.prop("disabled", true).empty().trigger("change");
};

NX.projectProformaInvoiceToggle = function (e, $self) {
    var project_id = e.params.data.id;
    var proformainvoice_feed_ref = $self.attr('data-feed-request-type');
    var proformainvoice_dropdown = $("#" + $self.attr('data-proformainvoice-dropdown'));
    ...
    // Show loading effect
    $(".dynamic_" + $self.attr('data-proformainvoice-dropdown')).addClass('loading');
    $self.prop("disabled", true);

    // Fetch workorders for the selected quotation
    $.ajax({
        type: 'GET',
        url: NX.site_url + "/feed/proforma-invoice?ref=" + proformainvoice_feed_ref + "&project_id=" + project_id
    }).done(function (proformainvoice) {
        if (proformainvoice.length > 0) {
            proformainvoice.forEach(function (item) {
                proformainvoice_dropdown.append(new Option(item.value, item.id, false, false));
            });

            proformainvoice_dropdown.prop("disabled", false).trigger("change");
        }
    }).fail(function () {
        console.log("Workorder Fetch Error");
    }).always(function () {
        $(".dynamic_" + $self.attr('data-proformainvoice-dropdown')).removeClass('loading');
        $self.prop("disabled", false);
    });
};
...
// --------------------------------- For Deliverables ---------------------------------
NX.addNewTechStackForDeliverables = function () {
    // Find #row-count inside .tech-stacks-container, not relative to $self (button)
    let $container = $('.tech-stacks-container');
    let prevCount = parseInt($container.find('#row-count').val()) || 0; // Default to 0 if undefined
    let currentCount = prevCount + 1;

    // Update the row count
    $container.find('#row-count').val(currentCount);

    const row = `
        <div class="form-group row tech-stack-row">
            <input type="hidden" class="deliverables_lineentry_id" name="deliverable[${currentCount}][deliverables_lineentry_id]" id="deliverable-${currentCount}-deliverables_lineentry_id" value="NA" />
            <div class="col-9">
                <select name="deliverable[${currentCount}][techstacks]" id="deliverable-${currentCount}-techstacks" class="form-control form-control-sm js-select2-basic-search select2-hidden-accessible"
                    data-ajax--url="/feed/techstack"></select>
            </div>
            <div class="col-3 text-right">
                <button type="button" class="btn sl-icon-trash-border waves-effect waves-dark remove-tech-stack" data-toggle="tooltip" title="">
    ...
    $(`#deliverable-${currentCount}-techstacks`).select2({
        theme: "bootstrap",
        width: null,
        containerCssClass: ':all:',
        minimumInputLength: 1,
...
NX.removeTechStackForDeliverables = function ($self) {
    let $container = $('.tech-stacks-container');
    let rowCount = $container.find('.tech-stack-row').length;

    if (rowCount > 1) {
        let $row = $self.closest('.tech-stack-row');
        let deliverableLineentryId = $row.find('.deliverables_lineentry_id').val();
        if (deliverableLineentryId !== "NA") {
            var confirm_popup_theme_style = 'red';

            //the confirm text
            var confirm_popup_button_class = ' btn-sm btn-outline-danger';
            var confirm_popup_text = '<div class="x-details">' + $self.data('content') + '</div>';

            $.confirm({
                type: confirm_popup_theme_style,
                title: $self.data('title'),
                content: confirm_popup_text,
                buttons: {
                    no: {
                        text: NXLANG.cancel,
                        btnClass: ' btn-sm btn-outline-secondary',
                    ...
                    yes: {
                        ...
                        action: function () {
                            $.ajax({
                                type: 'GET',
                                url: NX.site_url + "/settings/deliverables/delete-lineentry?deliverables_lineentry_id=" + deliverableLineentryId
                            }).then(function (data) {
                                if (data?.status) {
                                    $row.remove();
                                }
...
NX.toggleEditTaskChecklist = function ($self) {
    ...
    if (toggle == 'new') {
        ...
        $cloned.find('input[name="type"]').val($self.attr('data-type'));
        $(".checklist_text").html('');
