...
$(document).ready(function () {
    ...
    $(document).on("select2:unselecting", ".clients_and_quotation_toggle", function (e) {
        NX.clientAndQuotationClearToggle($(this));
    });
    //client list has been reset or cleared
    $(document).on("select2:select", ".clients_and_quotation_toggle", function (e) {
        NX.clientAndQuotationToggle(e, $(this));
    });

    $(document).on("select2:unselecting", ".filter_clients_and_quotation_toggle", function (e) {
        NX.filterClientAndQuotationClearToggle($(this));
    });
    //client list has been reset or cleared
    $(document).on("select2:select", ".filter_clients_and_quotation_toggle", function (e) {
        NX.filterClientAndQuotationToggle(e, $(this));
    });

    $(document).on("select2:unselecting", ".proforma_invoice_clients_and_quotation_toggle", function (e) {
        NX.clientQuotationWorkorderClearToggle($(this));
    });
    //client list has been reset or cleared
    $(document).on("select2:select", ".proforma_invoice_clients_and_quotation_toggle", function (e) {
        NX.clientQuotationWorkorderToggle(e, $(this));
    });



    $(document).on("select2:unselecting", ".quotation_and_workorder_toggle", function (e) {
        NX.quotationWorkorderClearToggle($(this));
    });
    //client list has been reset or cleared
    $(document).on("select2:select", ".quotation_and_workorder_toggle", function (e) {
        NX.quotationWorkorderToggle(e, $(this));
    });

    //  For Invoice
    $(document).on("select2:select", ".clients_projects_proformainvoice_toggle", function (e) {

        NX.clientProjectProformaInvoiceToggle(e, $(this));
    });
    $(document).on("select2:unselecting", ".clients_projects_proformainvoice_toggle", function (e) {
        NX.clientProjectProformaInvoiceClearToggle($(this));
    });
    $(document).on("select2:select", ".projects_proformainvoice_toggle", function (e) {
        NX.projectProformaInvoiceToggle(e, $(this));
    });
    $(document).on("select2:unselecting", ".projects_proformainvoice_toggle", function (e) {
        NX.projectProformaInvoiceClearToggle($(this));
    });

    $(document).on("click", ".add-deliverable", function (e) {
        NX.addNewTechStackForDeliverables();
    });

    $(document).on('click', '.remove-tech-stack', function () {
        NX.removeTechStackForDeliverables($(this));
    });
    ...
    $(document).ready(function () {
        $(document).on('click', '#workorder-file-attachments-upload-button', function () {
            $("#workorder-file-attachments-wrapper").fadeOut();
            $("#workorder-file-attachments-dropzone-wrapper").fadeIn();

        });

        $(document).on('click', '#workorder-file-attachments-close-button', function (event) {
            event.stopPropagation(); // Prevents event bubbling
            $("#workorder-file-attachments-dropzone-wrapper").fadeOut();
            $("#workorder-file-attachments-wrapper").fadeIn();
        });


        // Delete file attachment
        $(document).on('click', '#delete-workorder-file-attachment', function () {
            var parent = $(this).attr('data-parent');
            $("#" + parent).hide();
            nxAjaxUxRequest($(this));
        });
    ...
    /*----------------------------------------------------------------
     *  Project Module [Hours & Minutes] - Validation
     * -----------------------------------------------------------*/
    $(document).on('change input', '.minutes_validation, .hours_validation', function () {
        var $this = $(this);
        var value = parseInt($this.val(), 10);

        if (isNaN(value)) value = 0

        if ($this.hasClass('minutes_validation')) {
            value = Math.max(0, Math.min(59, value));
        } else if ($this.hasClass('hours_validation')) {
    ...
    $(document).on('select2:select', ".lead_clientid", function () {
        try {
            if ($("#add_lead_address_section").length && $("#add_lead_address_toggle").length) {
                $("#add_lead_address_section").hide();
                $("#add_lead_address_toggle").hide();
            }
        } catch (error) {
    ...
    $(document).on('select2:unselect', ".lead_clientid", function () {
        try {
            if ($("#add_lead_address_toggle").length) {
                $("#add_lead_address_toggle").show();
            }

            if ($("#add_lead_address_section").length && $("#show_more_settings_leads2").length) {
                if ($('#show_more_settings_leads2').is(':checked')) {
                    $("#add_lead_address_section").show();
                } else {
                    $("#add_lead_address_section").hide();
                }
            }
        } catch (error) {
            console.error("Error in select2:unselect handler:", error);
        }
    });
