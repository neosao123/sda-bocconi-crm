...
class ProjectRepository
{
    ...
    public function autocompleteQuotationProjectsFeed($status = '', $limit = '', $quotation_id = '', $searchterm = '')
    {
        //start
        $query = $this->projects->newQuery();
        $query->selectRaw("project_title AS value, project_id AS id");
        ...
        if (is_numeric($quotation_id)) {
            $quotation = \App\Models\Estimate::where('bill_estimateid', $quotation_id)->first();
            $query->where('project_id', '=', $quotation->bill_projectid);
        }
