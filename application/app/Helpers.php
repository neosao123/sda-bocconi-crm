 ...
 * @return bool
 */
function flipSortingUrl_old($url = '', $current_sorting = '')
{
...
/**
 * Summary of flipSortingUrl - Solve Description replace issue
 * @param mixed $url
 * @param mixed $current_sorting
 */
function flipSortingUrl($url = '', $current_sorting = '')
{
    // No valid sorting in URL
    if (!in_array($current_sorting, ['asc', 'desc'])) {
        return $url;
    }

    // Flip the sorting order properly by replacing the sortorder parameter only
    $new_sorting = ($current_sorting === 'asc') ? 'desc' : 'asc';

    // Use regex to replace only the sortorder query parameter value
    return preg_replace('/([?&]sortorder=)(asc|desc)/', '${1}' . $new_sorting, $url);
}
...
function runtimeTaskPriorityColors($value = '', $type = 'label')
{
    ...
    // dd(config('settings.task_priority'));
    //default colour
    $colour = 'default';
 ...
 * @return string checked | null
 */
function runtimeClientIdFormat($leadId = '')
{
    //add the zero's
    $prefix = config('system.settings_client_prefix');
    //return
    if (is_numeric($leadId)) {
        return $prefix . str_pad($leadId, 4, '0', STR_PAD_LEFT);
    } else {
        return '---';
    }
}
...
function runtimeCustomerIdFormat($customerId = '')
{
    //add the zero's
    $prefix = config('system.settings_customer_prefix');
    //return
    if (is_numeric($customerId)) {
        return $prefix . str_pad($customerId, 4, '0', STR_PAD_LEFT);
    } else {
        return '---';
    }
}
...
function customFieldValue($name = '', $obj = '', $type = 'text')
{
    ...
    if ($type == 'text' || $type == "paragraph") {
        if ($type == 'text' || $type == 'paragraph') {
            $text = $obj[$name] == '' ? '---' : strip_tags(html_entity_decode($obj[$name]));
 ...
 * @return string css setting
 */
function runtimeMoneyFormatPDF($number = 0, $span_id = "")
{
...
/**
 * Summary of convertNumberToWords
 * @param mixed $number
 * @return string
...
function convertNumberToWords($number)
{
    ...
    if ($number >= 10000000) {
        $result .= convertNumberToWords(floor($number / 10000000)) . ' Crore ';
        $number %= 10000000;
    }

    if ($number >= 100000) {
        $result .= convertNumberToWords(floor($number / 100000)) . ' Lakh ';
        $number %= 100000;
    }
    ...
    if ($number >= 100) {
        $result .= convertNumberToWords(floor($number / 100)) . ' Hundred ';
        $number %= 100;
    }
...
function runtimePDFCharacters($value = '')
{
    ...
    // ----------------------- New Code ------------------
    // Map symbols to safe HTML representations
    $symbolMap = [
        '$'  => '<span style="font-family:dejavusans;">$</span>',           // USD, AUD, etc.
        '€'  => '<span style="font-family:dejavusans;">&euro;</span>',      // EUR
        '¥'  => '<span style="font-family:dejavusans;">&yen;</span>',       // JPY
        '£'  => '<span style="font-family:dejavusans;">&pound;</span>',     // GBP
        'د.إ' => '<span style="font-family:dejavusans;">د.إ</span>',         // AED
        '؋'  => '<span style="font-family:dejavusans;">؋</span>',           // AFN
        'L'  => '<span style="font-family:dejavusans;">L</span>',           // ALL
        '֏'  => '<span style="font-family:dejavusans;">֏</span>',           // AMD
        'ƒ'  => '<span style="font-family:dejavusans;">ƒ</span>',           // ANG, AWG
        'Kz' => '<span style="font-family:dejavusans;">Kz</span>',          // AOA
        '₼'  => '<span style="font-family:dejavusans;">₼</span>',           // AZN
        'KM' => '<span style="font-family:dejavusans;">KM</span>',          // BAM
        '৳'  => '<span style="font-family:dejavusans;">৳</span>',           // BDT
        'лв' => '<span style="font-family:dejavusans;">лв</span>',          // BGN
        'ب.د' => '<span style="font-family:dejavusans;">ب.د</span>',         // BHD
        'FBu' => '<span style="font-family:dejavusans;">FBu</span>',         // BIF
        'Bs.' => '<span style="font-family:dejavusans;">Bs.</span>',        // BOB
        'R$' => '<span style="font-family:dejavusans;">R$</span>',          // BRL
        'Nu.' => '<span style="font-family:dejavusans;">Nu.</span>',        // BTN
        'P'  => '<span style="font-family:dejavusans;">P</span>',           // BWP
        'Br' => '<span style="font-family:dejavusans;">Br</span>',          // BYN
        'BZ$' => '<span style="font-family:dejavusans;">BZ$</span>',         // BZD
        'CA$'  => '<span style="font-family:dejavusans;">CA$</span>',    // CAD - Canadian dollar
        'FC'   => '<span style="font-family:dejavusans;">FC</span>',     // CDF - Congolese franc
        'CHF'  => '<span style="font-family:dejavusans;">CHF</span>',    // CHF - Swiss franc
        'CLP$' => '<span style="font-family:dejavusans;">CLP$</span>',   // CLP - Chilean peso
        '¥'    => '<span style="font-family:dejavusans;">&yen;</span>',  // CNY - Chinese yuan
        'COL$' => '<span style="font-family:dejavusans;">COL$</span>',   // COP - Colombian peso
        '₡'    => '<span style="font-family:dejavusans;">₡</span>',      // CRC - Costa Rican colón
        'CUC$' => '<span style="font-family:dejavusans;">CUC$</span>',   // CUC - Cuban convertible peso
        'CUP$' => '<span style="font-family:dejavusans;">CUP$</span>',   // CUP - Cuban peso
        'Esc'  => '<span style="font-family:dejavusans;">Esc</span>',    // CVE - Cape Verdean escudo
        'Kč'   => '<span style="font-family:dejavusans;">Kč</span>',     // CZK - Czech koruna
        'Fdj'  => '<span style="font-family:dejavusans;">Fdj</span>',    // DJF - Djiboutian franc
        'kr'   => '<span style="font-family:dejavusans;">kr</span>',     // DKK - Danish krone
        'RD$'  => '<span style="font-family:dejavusans;">RD$</span>',    // DOP - Dominican peso
        'دج'   => '<span style="font-family:dejavusans;">دج</span>',     // DZD - Algerian dinar
        'E£'   => '<span style="font-family:dejavusans;">E£</span>',     // EGP - Egyptian pound
        'Nfk'  => '<span style="font-family:dejavusans;">Nfk</span>',    // ERN - Eritrean nakfa
        'Br'   => '<span style="font-family:dejavusans;">Br</span>',     // ETB - Ethiopian birr
        '€'    => '<span style="font-family:dejavusans;">&euro;</span>', // EUR - Euro
        'FJ$'  => '<span style="font-family:dejavusans;">FJ$</span>',    // FJD - Fijian dollar
        '£'    => '<span style="font-family:dejavusans;">&pound;</span>', // FKP - Falkland Islands pound
        '£'    => '<span style="font-family:dejavusans;">&pound;</span>', // GBP - British pound
        '₾'    => '<span style="font-family:dejavusans;">₾</span>',      // GEL - Georgian lari
        '£'    => '<span style="font-family:dejavusans;">&pound;</span>', // GGP - Guernsey pound
        'GH₵'  => '<span style="font-family:dejavusans;">GH₵</span>',    // GHS - Ghanaian cedi
        '£'    => '<span style="font-family:dejavusans;">&pound;</span>', // GIP - Gibraltar pound
        'D'    => '<span style="font-family:dejavusans;">D</span>',      // GMD - Gambian dalasi
        'FG'   => '<span style="font-family:dejavusans;">FG</span>',     // GNF - Guinean franc
        'Q'    => '<span style="font-family:dejavusans;">Q</span>',      // GTQ - Guatemalan quetzal
        'GY$'  => '<span style="font-family:dejavusans;">GY$</span>',    // GYD - Guyanese dollar
        'HK$'  => '<span style="font-family:dejavusans;">HK$</span>',    // HKD - Hong Kong dollar
        'L'    => '<span style="font-family:dejavusans;">L</span>',      // HNL - Honduran lempira
        'kn'   => '<span style="font-family:dejavusans;">kn</span>',     // HRK - Croatian kuna
        'G'    => '<span style="font-family:dejavusans;">G</span>',      // HTG - Haitian gourde
        'Ft'   => '<span style="font-family:dejavusans;">Ft</span>',     // HUF - Hungarian forint
        'Rp'   => '<span style="font-family:dejavusans;">Rp</span>',     // IDR - Indonesian rupiah
        '₪'    => '<span style="font-family:dejavusans;">&#8362;</span>', // ILS - Israeli new shekel
        '£'    => '<span style="font-family:dejavusans;">&pound;</span>', // IMP - Manx pound
        '₹'    => '<span style="font-family:dejavusans;">&#8377;</span>', // INR - Indian rupee
        'ع.د'  => '<span style="font-family:dejavusans;">ع.د</span>',    // IQD - Iraqi dinar
        '﷼'    => '<span style="font-family:dejavusans;">﷼</span>',      // IRR - Iranian rial
        'kr'   => '<span style="font-family:dejavusans;">kr</span>',     // ISK - Icelandic króna
        '£'    => '<span style="font-family:dejavusans;">&pound;</span>', // JEP - Jersey pound
        'J$'   => '<span style="font-family:dejavusans;">J$</span>',     // JMD - Jamaican dollar
        'د.ا'  => '<span style="font-family:dejavusans;">د.ا</span>',    // JOD - Jordanian dinar
        '¥'    => '<span style="font-family:dejavusans;">&yen;</span>',  // JPY - Japanese yen
        'KSh'  => '<span style="font-family:dejavusans;">KSh</span>',    // KES - Kenyan shilling
        'сом'  => '<span style="font-family:dejavusans;">сом</span>',    // KGS - Kyrgyzstani som
        '៛'    => '<span style="font-family:dejavusans;">&#6107;</span>', // KHR - Cambodian riel
        'KID$' => '<span style="font-family:dejavusans;">KID$</span>',   // KID - Kiribati dollar
        'CF'   => '<span style="font-family:dejavusans;">CF</span>',     // KMF - Comorian franc
        '₩'    => '<span style="font-family:dejavusans;">&#8361;</span>', // KPW - North Korean won
        '₩'    => '<span style="font-family:dejavusans;">&#8361;</span>', // KRW - South Korean won
        'د.ك'  => '<span style="font-family:dejavusans;">د.ك</span>',    // KWD - Kuwaiti dinar
        'CI$'  => '<span style="font-family:dejavusans;">CI$</span>',    // KYD - Cayman Islands dollar
        '₸'    => '<span style="font-family:dejavusans;">&#8376;</span>', // KZT - Kazakhstani tenge
        '₭'    => '<span style="font-family:dejavusans;">₭</span>',      // LAK - Lao kip
        'ل.ل'  => '<span style="font-family:dejavusans;">ل.ل</span>',    // LBP - Lebanese pound
        'Rs'   => '<span style="font-family:dejavusans;">Rs</span>',     // LKR - Sri Lankan rupee
        'L$'   => '<span style="font-family:dejavusans;">L$</span>',     // LRD - Liberian dollar
        'L'    => '<span style="font-family:dejavusans;">L</span>',      // LSL - Lesotho loti
        'ل.د'  => '<span style="font-family:dejavusans;">ل.د</span>',    // LYD - Libyan dinar
        'د.م.'  => '<span style="font-family:dejavusans;">د.م.</span>',    // MAD - Moroccan dirham
        'L'     => '<span style="font-family:dejavusans;">L</span>',       // MDL - Moldovan leu
        'Ar'    => '<span style="font-family:dejavusans;">Ar</span>',      // MGA - Malagasy ariary
        'ден'   => '<span style="font-family:dejavusans;">ден</span>',     // MKD - Macedonian denar
        'K'     => '<span style="font-family:dejavusans;">K</span>',       // MMK - Burmese kyat
        '₮'     => '<span style="font-family:dejavusans;">₮</span>',       // MNT - Mongolian tögrög
        'MOP$'  => '<span style="font-family:dejavusans;">MOP$</span>',    // MOP - Macanese pataca
        'UM'    => '<span style="font-family:dejavusans;">UM</span>',      // MRU - Mauritanian ouguiya
        '₨'     => '<span style="font-family:dejavusans;">₨</span>',       // MUR - Mauritian rupee
        'ރ.'    => '<span style="font-family:dejavusans;">ރ.</span>',      // MVR - Maldivian rufiyaa
        'MK'    => '<span style="font-family:dejavusans;">MK</span>',      // MWK - Malawian kwacha
        '$'     => '<span style="font-family:dejavusans;">$</span>',       // MXN - Mexican peso
        'RM'    => '<span style="font-family:dejavusans;">RM</span>',      // MYR - Malaysian ringgit
        'MT'    => '<span style="font-family:dejavusans;">MT</span>',      // MZN - Mozambican metical
        'N$'    => '<span style="font-family:dejavusans;">N$</span>',      // NAD - Namibian dollar
        '₦'     => '<span style="font-family:dejavusans;">₦</span>',       // NGN - Nigerian naira
        'C$'    => '<span style="font-family:dejavusans;">C$</span>',      // NIO - Nicaraguan córdoba
        'kr'    => '<span style="font-family:dejavusans;">kr</span>',      // NOK - Norwegian krone
        'रू'    => '<span style="font-family:dejavusans;">रू</span>',      // NPR - Nepalese rupee
        'NZ$'   => '<span style="font-family:dejavusans;">NZ$</span>',     // NZD - New Zealand dollar
        'ر.ع.'  => '<span style="font-family:dejavusans;">ر.ع.</span>',    // OMR - Omani rial
        'B/.'   => '<span style="font-family:dejavusans;">B/.</span>',     // PAB - Panamanian balboa
        'S/'    => '<span style="font-family:dejavusans;">S/</span>',      // PEN - Peruvian sol
        'K'     => '<span style="font-family:dejavusans;">K</span>',       // PGK - Papua New Guinean kina
        '₱'     => '<span style="font-family:dejavusans;">&#8369;</span>', // PHP - Philippine peso
        '₨'     => '<span style="font-family:dejavusans;">₨</span>',       // PKR - Pakistani rupee
        'zł'    => '<span style="font-family:dejavusans;">zł</span>',      // PLN - Polish złoty
        'р.'    => '<span style="font-family:dejavusans;">р.</span>',      // PRB - Transnistrian ruble
        '₲'     => '<span style="font-family:dejavusans;">₲</span>',       // PYG - Paraguayan guaraní
        'ر.ق'   => '<span style="font-family:dejavusans;">ر.ق</span>',     // QAR - Qatari riyal
        'lei'   => '<span style="font-family:dejavusans;">lei</span>',     // RON - Romanian leu (duplicate removed)
        'дин'   => '<span style="font-family:dejavusans;">дин</span>',     // RSD - Serbian dinar
        '₽'     => '<span style="font-family:dejavusans;">₽</span>',       // RUB - Russian ruble
        'FRw'   => '<span style="font-family:dejavusans;">FRw</span>',     // RWF - Rwandan franc
        'ر.س'    => '<span style="font-family:dejavusans;">ر.س</span>',     // SAR - Saudi riyal
        'kr'     => '<span style="font-family:dejavusans;">kr</span>',      // SEK - Swedish krona
        'S$'     => '<span style="font-family:dejavusans;">S$</span>',      // SGD - Singapore dollar
        '£'      => '<span style="font-family:dejavusans;">£</span>',       // SHP - Saint Helena pound
        'Le'     => '<span style="font-family:dejavusans;">Le</span>',      // SLL - Sierra Leonean leone
        'SLS'    => '<span style="font-family:dejavusans;">SLS</span>',     // SLS - Somaliland shilling (not ISO-standard, custom)
        'Sh'     => '<span style="font-family:dejavusans;">Sh</span>',      // SOS - Somali shilling
        '$'      => '<span style="font-family:dejavusans;">$</span>',       // SRD - Surinamese dollar
        '£'      => '<span style="font-family:dejavusans;">£</span>',       // SSP - South Sudanese pound
        'Db'     => '<span style="font-family:dejavusans;">Db</span>',      // STN - São Tomé and Príncipe dobra
        '£'      => '<span style="font-family:dejavusans;">£</span>',       // SYP - Syrian pound
        'E'      => '<span style="font-family:dejavusans;">E</span>',       // SZL - Swazi lilangeni
        '฿'      => '<span style="font-family:dejavusans;">฿</span>',       // THB - Thai baht
        'ЅМ'     => '<span style="font-family:dejavusans;">ЅМ</span>',      // TJS - Tajikistani somoni
        'T'      => '<span style="font-family:dejavusans;">T</span>',       // TMT - Turkmenistan manat
        'د.ت'    => '<span style="font-family:dejavusans;">د.ت</span>',     // TND - Tunisian dinar
        'T$'     => '<span style="font-family:dejavusans;">T$</span>',      // TOP - Tongan paʻanga
        '₺'      => '<span style="font-family:dejavusans;">₺</span>',       // TRY - Turkish lira
        'TT$'    => '<span style="font-family:dejavusans;">TT$</span>',     // TTD - Trinidad and Tobago dollar
        'TVD$'   => '<span style="font-family:dejavusans;">TVD$</span>',    // TVD - Tuvaluan dollar (non-standard ISO)
        'NT$'    => '<span style="font-family:dejavusans;">NT$</span>',     // TWD - New Taiwan dollar
        'TSh'    => '<span style="font-family:dejavusans;">TSh</span>',     // TZS - Tanzanian shilling
        '₴'      => '<span style="font-family:dejavusans;">₴</span>',       // UAH - Ukrainian hryvnia
        'USh'    => '<span style="font-family:dejavusans;">USh</span>',     // UGX - Ugandan shilling
        '$'      => '<span style="font-family:dejavusans;">$</span>',       // USD - United States dollar
        '$U'     => '<span style="font-family:dejavusans;">$U</span>',      // UYU - Uruguayan peso
        'сўм'    => '<span style="font-family:dejavusans;">сўм</span>',     // UZS - Uzbekistani soʻm
        'Bs.S'   => '<span style="font-family:dejavusans;">Bs.S</span>',    // VES - Venezuelan bolívar soberano
        '₫'      => '<span style="font-family:dejavusans;">₫</span>',       // VND - Vietnamese đồng
        'VT'     => '<span style="font-family:dejavusans;">VT</span>',      // VUV - Vanuatu vatu
        'WS$'    => '<span style="font-family:dejavusans;">WS$</span>',     // WST - Samoan tālā
        'FCFA'   => '<span style="font-family:dejavusans;">FCFA</span>',    // XAF - Central African CFA franc
        '$'      => '<span style="font-family:dejavusans;">$</span>',       // XCD - Eastern Caribbean dollar
        'CFA'    => '<span style="font-family:dejavusans;">CFA</span>',     // XOF - West African CFA franc
        '₣'      => '<span style="font-family:dejavusans;">₣</span>',       // XPF - CFP franc
        'R'      => '<span style="font-family:dejavusans;">R</span>',       // ZAR - South African rand
        'ZK'     => '<span style="font-family:dejavusans;">ZK</span>',      // ZMW - Zambian kwacha
        'Z$'     => '<span style="font-family:dejavusans;">Z$</span>',      // ZWL - Zimbabwean Dollar
    ];
...
function runtimeBillTaxTypeURL($bill = '')
{
    ...
    if ($bill->bill_type == 'proforma-invoice') {
        return url('/proforma-invoices/' . $bill->bill_proforma_invoiceid . '/change-tax-type');
    }
}
...
function runtimeURLBillAttachFiles($bill = '')
{
    ...
    //proforma-invoice
    if ($bill->bill_type == 'proforma-invoice') {
        return url('/proforma-invoices/' . $bill->bill_proforma_invoiceid . '/attach-files');
    }
...
function getHoursMinutes($total_minutes, $type)
{
...
function getLeadId($lead)
{
    if ($lead->lead_clientid) {
        $leadId = $lead->customer_lead_id;
        $type = 'client';
        $formatted_id = $lead->formatted_customerid;
    } else {
        $leadId = $lead->lead_id;
        $type = 'lead';
        $formatted_id = $lead->formatted_clientid;
    }
...
function checkTeamMemberVisibility($teamMember, $leadType)
{
    if ($leadType == 'inhouse') {
        return $teamMember->role_id != 4;
    } elseif ($leadType == 'associate') {
        return $teamMember->role_id == 4;
    }
    return false;
}
...
{
    $startDayMonth = explode('-', env('FINANCIAL_YEAR_START_DATE', '01-04'));
    $startDay = (int)$startDayMonth[0];
    $startMonth = (int)$startDayMonth[1];

    $now = now();

    if ($now->month < $startMonth || ($now->month == $startMonth && $now->day < $startDay)) {
        $fyStart = \Carbon\Carbon::create($now->year - 1, $startMonth, $startDay);
        $fyEnd   = \Carbon\Carbon::create($now->year, $startMonth, $startDay)->subDay();
    } else {
        $fyStart = \Carbon\Carbon::create($now->year, $startMonth, $startDay);
        $fyEnd   = \Carbon\Carbon::create($now->year + 1, $startMonth, $startDay)->subDay();
    }
...
function normalizeCompanyName(?string $name): string
{
    ...
    // Replace variations of "Private Limited" with "Pvt. Ltd"
    return preg_replace('/\b(private\s+limited)\b/i', 'Pvt. Ltd', $name);
}
