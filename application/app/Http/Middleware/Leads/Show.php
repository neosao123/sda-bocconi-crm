...
class Show
{
     ...
     *
     */
    public function __construct(LeadPermissions $leadpermissions, Lead $leadmodel, CustomerLeads $customerleadmodel, LeadRepository $leadrepo)
    {
    ...
    public function handle($request, Closure $next)
    {
        ...
        if ($request->type == "lead") {
            ...
            if (!$lead = $this->customerleadmodel::find($lead_id)) {
                Log::error("lead could not be found", ['process' => '[permissions][leads][destroy]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'lead id' => $lead_id ?? '']);
                abort(404);
            }
        }
