...
class Tasks extends Controller
{
    ...
    public function store(TaskStoreUpdate $request, TaskAssignedRepository $assignedrepo)
    {
        ...
        if (!request()->filled('task_milestoneid')) {
            $projectId = request('task_projectid');

            // Try to find an uncategorised milestone first
            $milestone = \App\Models\Milestone::where('milestone_type', 'uncategorised')
                ->where('milestone_projectid', $projectId)
                ->first();

            // If not found, try to get the latest milestone for that project
            if (!$milestone) {
                $milestone = \App\Models\Milestone::where('milestone_projectid', $projectId)
                    ->latest()
                    ->first();
            }

            // If milestone found, merge it into the request
            if ($milestone) {
                request()->merge(['task_milestoneid' => $milestone->milestone_id]);
            } else {
                Log::critical('Add task - default milestone could not be found', ['process' => '[tasks]', 'debug_ref' => config('app.debug_ref'), 'function' => __FUNCTION__, 'file' => basename(__FILE__), 'line' => __LINE__, 'path' => __FILE__, 'project_id' => $projectId]);
                abort(409, __('lang.milestone_not_found'));
            }
        }
    ...
    public function updateStatus(ProjectPermissions $projectpermissions, $id)
    {
        ...
        if (!$this->taskpermissions->check('view', $task)) {
            $this->processTasks($tasks);
            $payload = [
                'task' => $tasks->first(),
                'tasks' => $tasks,
