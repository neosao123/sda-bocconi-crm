...
class LeadPermissions
{
    ...
    public function check($action = '', $lead = '')
    {
        ...
        //GET THE RESOURCE
        if (is_numeric($lead)) {
            $lead = request()->type == "client" ? \App\Models\CustomerLeads::Where('customer_lead_id', $lead)->first() : \App\Models\Lead::Where('lead_id', $lead)->first();
            if (!$lead) {
        ...
        if ($lead instanceof \App\Models\Lead || $lead instanceof \Illuminate\Pagination\LengthAwarePaginator || $lead instanceof \App\Models\CustomerLeads) {
        ...
        if ($action == 'participate') {
            ...
            // dd($assigned_users->contains(auth()->id()));
            if ($assigned_users->contains(auth()->id())) {
                return true;
            }
