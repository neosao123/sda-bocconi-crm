      ...
      <div class="x-section">
        ...
        @if (config('visibility.tasks_standard_features'))
          <div class="x-element" id="task-start-date"><i class="mdi mdi-calendar-plus"></i>
            ...
            @if ($task->permission_edit_task)
              <span class="x-highlight x-editable card-pickadate" data-url="{{ urlResource('/tasks/' . $task->task_id . '/update-start-date/') }}" data-type="form" data-progress-bar='hidden'
        ...
        @if (config('visibility.tasks_standard_features'))
          <div class="x-element" id="task-due-date"><i class="mdi mdi-calendar-clock"></i>
            ...
            @if ($task->permission_edit_task)
              <span class="x-highlight x-editable card-pickadate" data-url="{{ urlResource('/tasks/' . $task->task_id . '/update-due-date/') }}" data-type="form" data-progress-bar='hidden'
        ...
        <div class="x-element" id="card-task-status"><i class="mdi mdi-flag"></i>
          ...
          <span class="x-highlight x-editable js-card-settings-button-static" data-container=".card-modal" id="card-task-status-text" tabindex="0" data-popover-content="card-task-statuses"
            data-offset="0 25%" data-status-id="{{ $task->taskstatus_id }}" data-title="{{ cleanLang(__('lang.status')) }}">{{ runtimeLang($task->taskstatus_title) }}</strong></span>
          {{-- @else --}}
        ...
        <div class="x-element" id="card-task-priority"><i class="mdi mdi-flag"></i>
          ...
          @if ($task->permission_edit_task)
            <span class="x-highlight x-editable js-card-settings-button-static" data-container=".card-modal" id="card-task-priority-text" tabindex="0" data-popover-content="card-task-priorities"
              data-offset="0 25%" data-status-id="{{ $task->taskpriority_id }}" data-title="{{ cleanLang(__('lang.priority')) }}">{{ runtimeLang($task->taskpriority_title) }}</strong></span>
          @else
    ...
    @if (config('visibility.task_is_open'))
      ...
      <div class="x-section">
        ...
        @if ($task->permission_super_user && config('visibility.tasks_standard_features'))
          <div class="x-element x-action confirm-action-danger" data-confirm-title="{{ cleanLang(__('lang.stop_all_timers')) }}" data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}"
            data-url="{{ url('/') }}/tasks/timer/{{ $task->task_id }}/stopall?source=card"><i class="mdi mdi-timer-off"></i>
        ...
        @if ($task->permission_edit_task && config('visibility.tasks_standard_features'))
          <div class="x-element x-action confirm-action-info  {{ runtimeActivateOrAchive('archive-button', $task->task_active_state) }} card_archive_button_{{ $task->task_id }}"
            id="card_archive_button_{{ $task->task_id }}" data-confirm-title="{{ cleanLang(__('lang.archive_task')) }}" data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}"
            data-ajax-type="PUT" data-url="{{ url('/') }}/tasks/{{ $task->task_id }}/archive"><i class="mdi mdi-archive"></i> <span class="x-highlight"
        ...
        @if ($task->permission_edit_task && runtimeArchivingOptions())
          <div class="x-element x-action confirm-action-info  {{ runtimeActivateOrAchive('activate-button', $task->task_active_state) }} card_restore_button_{{ $task->task_id }}"
            id="card_restore_button_{{ $task->task_id }}" data-confirm-title="{{ cleanLang(__('lang.restore_task')) }}" data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}"
            data-ajax-type="PUT" data-url="{{ url('/') }}/tasks/{{ $task->task_id }}/activate"><i class="mdi mdi-archive"></i> <span class="x-highlight"
        ...
        @if ($task->permission_delete_task && runtimeArchivingOptions())
          <div class="x-element x-action confirm-action-danger" data-confirm-title="{{ cleanLang(__('lang.delete_item')) }}" data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}"
            data-ajax-type="DELETE" data-url="{{ urlResource('/') }}/tasks/{{ $task->task_id }}"><i class="mdi mdi-delete"></i> <span class="x-highlight"
              id="task-start-date">{{ cleanLang(__('lang.delete')) }}</span></span></div>
          @php $count_action ++ ; @endphp
        @endif
            ...
            <tbody>
              ...
              @if (auth()->user()->is_team)
                <tr>
                  ...
                  <td><strong><span id="task_timer_all_card_{{ $task->task_id }}">{!! clean(runtimeSecondsHumanReadable($task->sum_all_time, false)) !!}</span></strong>
                  </td>
                </tr>
                <tr>
                  <td>{{ cleanLang(__('lang.time_invoiced')) }}</td>
                  <td><strong><span id="task_timer_all_card_{{ $task->task_id }}">{!! clean(runtimeSecondsHumanReadable($task->sum_invoiced_time, false)) !!}</span></strong>
                  </td>
                </tr>
                <tr>
                  <td>{{ cleanLang(__('lang.project')) }}</td>
                  <td><strong><a href="{{ urlResource('/projects/' . $task->task_projectid) }}" target="_blank">#{{ $task->project_id }}</a></strong>
                  </td>
                </tr>
              @endif
            </tbody>
    ...
    <div class="hidden" id="card-task-statuses">
      <ul class="list">
        @foreach (config('task_statuses') as $task_status)
          <li class="card-tasks-update-status-link" data-button-text="card-task-status-text" data-progress-bar='hidden' data-url="{{ urlResource('/tasks/' . $task->task_id . '/update-status') }}"
        ...
        <ul class="list">
          @foreach (config('task_priorities') as $priority)
            <li class="card-tasks-update-priority-link" data-button-text="card-task-priority-text" data-progress-bar='hidden'
              data-url="{{ urlResource('/tasks/' . $task->task_id . '/update-priority') }}" data-type="form" data-value="{{ $priority->taskpriority_id }}" data-form-id="--set-dynamically--"
              data-ajax-type="post">
      ...
      <div class="hidden" id="card-task-priorities">
        <ul class="list">
          @foreach (config('settings.task_priority') as $key => $value)
            <li class="card-tasks-update-priority-link" data-button-text="card-task-priority-text" data-progress-bar='hidden'
              data-url="{{ urlResource('/tasks/' . $task->task_id . '/update-priority') }}" data-type="form" data-value="{{ $key }}" data-form-id="--set-dynamically--"
              data-ajax-type="post">
              {{ runtimeLang($key) }}</li>
          @endforeach
        </ul>
      ...
      <div class="hidden" id="card-task-visibility">
        <ul class="list">
          <li class="card-tasks-update-visibility-link" data-button-text="card-task-client-visibility-text" data-progress-bar='hidden'
            data-url="{{ urlResource('/tasks/' . $task->task_id . '/update-visibility') }}" data-type="form" data-value="no" data-text="{{ cleanLang(__('lang.hidden')) }}"
            data-form-id="card-task-client-visibility" data-ajax-type="post">
            {{ cleanLang(__('lang.hidden')) }}
          </li>
          <li class="card-tasks-update-visibility-link" data-button-text="card-task-client-visibility-text" data-progress-bar='hidden'
            data-url="{{ urlResource('/tasks/' . $task->task_id . '/update-visibility') }}" data-type="form" data-value="yes" data-text="{{ cleanLang(__('lang.visible')) }}"
            data-form-id="card-task-client-visibility" data-ajax-type="post">
      ...
      <div class="hidden" id="card-task-milestones">
        ...
        <div class="form-group text-right">
          <button type="button" class="btn btn-danger btn-sm" id="card-tasks-update-milestone-button" data-progress-bar='hidden'
            data-url="{{ urlResource('/tasks/' . $task->task_id . '/update-milestone') }}" data-type="form" data-ajax-type="post" data-form-id="popover-body">
      ...
      <div class="card-assigned-popover-content">
        ...
        @foreach ($project_assigned as $staff)
          <div class="form-check m-b-15">
            <label class="custom-control custom-checkbox">
              <input type="checkbox" name="assigned[{{ $staff->id }}]" class="custom-control-input assigned_user_{{ $staff->id }}">
              <span class="custom-control-indicator"></span>
              <span class="custom-control-description"><img src="{{ getUsersAvatar($staff->avatar_directory, $staff->avatar_filename) }}" class="img-circle avatar-xsmall">
                {{ $staff->first_name }}
        ...
        <div class="form-group text-right">
          <button type="button" class="btn btn-danger btn-sm" id="card-tasks-update-assigned" data-progress-bar='hidden'
            data-url="{{ urlResource('/tasks/' . $task->task_id . '/update-assigned') }}" data-type="form" data-ajax-type="post" data-form-id="popover-body">
            {{ cleanLang(__('lang.update')) }}
          </button>
        </div>
      </div>
