...
<div class="col-md-12  col-lg-7 p-b-9 align-self-center text-right {{ $page['list_page_actions_size'] ?? '' }} {{ $page['list_page_container_class'] ?? '' }}" id="list-page-actions-container">
  <div id="list-page-actions">
    ...
    <!--SHOW ARCHIVED LEADS-->
    <button type="button" data-toggle="tooltip" title="{{ cleanLang(__('lang.show_archive_leads')) }}" id="pref_filter_show_archived_leads"
      class="list-actions-button btn btn-page-actions archive-border waves-effect waves-dark js-ajax-ux-request {{ runtimeActive(auth()->user()->pref_filter_show_archived_leads) }}"
    ...
    @if (config('visibility.own_leads_toggle_button'))
      <button type="button" data-toggle="tooltip" title="{{ cleanLang(__('lang.my_leads')) }}" id="pref_filter_own_leads"
        class="list-actions-button btn btn-page-actions person-border waves-effect waves-dark js-ajax-ux-request {{ runtimeActive(auth()->user()->pref_filter_own_leads) }}"
    ...
    <button type="button" data-toggle="tooltip" title="{{ cleanLang(__('lang.kanban_view')) }}" id="pref_view_leads_layout"
      class="list-actions-button btn btn-page-actions list-border waves-effect waves-dark js-ajax-ux-request {{ runtimeActive(auth()->user()->pref_view_leads_layout) }}"
    ...
    @if (config('visibility.list_page_actions_importing'))
      <button type="button" title="{{ cleanLang(__('lang.import_leads')) }}" id="leads-import-button"
        ...
        data-target="#commonModal" data-footer-visibility="hidden" data-top-padding="none" data-action-url="{{ url('import/leads') }}" data-action-method="POST" data-loading-target="commonModalBody"
        data-action-ajax-loading-target="commonModalBody" data-modal-title="@lang('lang.import_leads')" data-url="{{ url('import/leads/create') }}">
    ...
    @if (config('visibility.list_page_actions_exporting'))
      <button type="button" data-toggle="tooltip" title="@lang('lang.export_leads')" class="list-actions-button btn btn-page-actions export-border  waves-effect waves-dark js-toggle-side-panel"
