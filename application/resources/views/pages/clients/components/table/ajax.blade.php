@foreach ($clients as $client)
  ...
  <tr id="client_{{ $client->client_id }}">
    ...
    <td class="clients_col_id {{ config('table.tableconfig_column_1') }} tableconfig_column_1" id="clients_col_id_{{ $client->client_id }}">
      <a href="/clients/{{ $client->client_id ?? '' }}">{{ $client->formatted_customerid }}</a>
    </td>

    <!--tableconfig_column_2 [client_company_name]-->
    <td class="clients_col_company {{ config('table.tableconfig_column_2') }} tableconfig_column_2" id="clients_col_id_{{ $client->client_id }}">
      <a href="/clients/{{ $client->client_id ?? '' }}">{{ str_limit($client->client_company_name, 35) }}</a>
    </td>

    <!--tableconfig_column_3 [account_owner]-->
    <td class="clients_col_account_owner {{ config('table.tableconfig_column_3') }} tableconfig_column_3" id="clients_col_account_owner_{{ $client->client_id }}">
      <img src="{{ getUsersAvatar($client->avatar_directory, $client->avatar_filename) }}" alt="user" class="img-circle avatar-xsmall">
    ...
    <td class="clients_col_action actions_column" id="clients_col_action_{{ $client->client_id }}">
      ...
      <span class="list-table-action dropdown font-size-inherit">
        ...
        @if (config('visibility.action_buttons_delete'))
          <button type="button" title="{{ cleanLang(__('lang.delete')) }}" class="data-toggle-action-tooltip btn  sl-icon-trash-border btn-outline-danger btn-circle btn-sm confirm-action-danger"
            data-confirm-title="{{ cleanLang(__('lang.delete_client')) }}" data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}" data-ajax-type="DELETE"
        ...
        @if (config('visibility.action_buttons_edit'))
          <button type="button" title="{{ cleanLang(__('lang.edit')) }}"
            ...
            data-target="#commonModal" data-url="{{ urlResource('/clients/' . $client->client_id . '/edit') }}" data-loading-target="commonModalBody"
            data-modal-title="{{ cleanLang(__('lang.edit_client')) }}" data-action-url="{{ urlResource('/clients/' . $client->client_id . '?ref=list') }}" data-action-method="PUT"
        ...
        <!--send email-->
        <button type="button" title="@lang('lang.send_email')" class="data-toggle-action-tooltip btn btn-outline-warning   btn-circle btn-sm edit-add-modal-button js-ajax-ux-request reset-target-modal-form"
          data-toggle="modal" data-target="#commonModal" data-url="{{ url('/appwebmail/compose?view=modal&webmail_template_type=clients&resource_type=client&resource_id=' . $client->client_id) }}"
