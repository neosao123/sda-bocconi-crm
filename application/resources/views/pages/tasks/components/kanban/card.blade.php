@foreach ($board['tasks'] as $task)
  ...
  <div class="kanban-card kanban-board-element show-modal-button reset-card-modal-form js-ajax-ux-request" data-toggle="modal" data-target="#cardModal"
    data-url="{{ urlResource('/tasks/' . $task->task_id) }}" data-task-id="{{ $task->task_id }}" data-loading-target="main-top-nav-bar" id="card_task_{{ $task->task_id }}">

    <!--cover image-->
    <div id="kanban-card-cover-image-{{ $task->task_id }}" class="kanban-card-cover-image {{ runtimeKanbanCoverImage($task->task_cover_image) }}" {!! runtimeCoverImage($task->task_cover_image_uniqueid, $task->task_cover_image_filename) !!}>

    </div>
    <div class="kanban-card-content-comntainer">
      <div class="x-title wordwrap" id="kanban_task_title_{{ $task->task_id }}">{{ $task->task_title }}
        <span class="x-action-button" id="card-action-button-{{ $task->task_id }}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="mdi mdi-dots-vertical"></i></span>
        <div class="dropdown-menu dropdown-menu-small dropdown-menu-right js-stop-propagation" aria-labelledby="card-action-button-{{ $task->task_id }}">
          ...
          @if ($task->permission_delete_task)
            <a class="dropdown-item confirm-action-danger  js-stop-propagation" data-confirm-title="{{ cleanLang(__('lang.delete_item')) }}"
              data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}" data-ajax-type="DELETE" data-url="{{ url('/') }}/tasks/{{ $task->task_id }}">{{ cleanLang(__('lang.delete')) }}</a>
          ...
          @if (auth()->user()->is_team && $task->permission_edit_task)
            <a class="dropdown-item edit-add-modal-button js-ajax-ux-request reset-target-modal-form" data-toggle="modal" data-target="#commonModal" data-modal-title="@lang('lang.clone_task')"
              data-url="{{ urlResource('/tasks/' . $task->task_id . '/clone') }}" data-action-url="{{ urlResource('/tasks/' . $task->task_id . '/clone') }}" data-modal-size="modal-lg"
          ...
          @if ($task->assigned_to_me)
            <a class="dropdown-item edit-add-modal-button js-ajax-ux-request reset-target-modal-form" data-confirm-title="{{ cleanLang(__('lang.archive_task')) }}" data-toggle="modal"
              data-target="#commonModal" data-modal-title="@lang('lang.record_your_work_time')" data-url="{{ url('/timesheets/create?task_id=' . $task->task_id) }}" data-action-url="{{ urlResource('/timesheets') }}"
      ...
      <div class="x-meta">
        ...
        @if (config('system.settings_tasks_kanban_milestone') == 'show')
          <div>
            <label class="label label-{{ $task->milestone_color ?? 'default' }} p-t-3 p-b-3 p-l-8 p-r-8" data-toggle="tooltip" title="@lang('lang.milestone')">{{ $task->milestone_title }}</label>
          </div>
        @endif
        <!--priority-->
        @if (config('system.settings_tasks_kanban_priority') == 'show')
          <label class="label label-{{ $task->taskpriority_color }} p-t-3 p-b-3 p-l-8 p-r-8" data-toggle="tooltip" title="@lang('lang.priority')">{{ $task->taskpriority_title }}</label>
        @endif
        <!--tags-->
      ...
      <div class="x-footer row">
        <div class="col-6 x-icons">
          ...
          @if ($task->task_creatorid == auth()->user()->id)
            <span class="x-icon text-info display-inline-block vm p-t-2" data-toggle="tooltip" title="@lang('lang.you_created_this_task')" data-placement="top"><i class="mdi mdi-account-circle"></i></span>
          @endif

          <!--[icon] archived-->
          @if (runtimeArchivingOptions())
            <span class="x-icon {{ runtimeActivateOrAchive('archived-icon', $task->task_active_state) }}" id="archived_icon_{{ $task->task_id }}" data-toggle="tooltip" title="@lang('lang.archived')">
          ...
          @if (config('system.settings_tasks_kanban_client_visibility') == 'show' && auth()->user()->is_team)
            @if ($task->task_client_visibility == 'no')
              <span class="x-icon display-inline-block vm " data-toggle="tooltip" title="{{ cleanLang(__('lang.client')) }} - {{ cleanLang(__('lang.hidden')) }}" data-placement="top"><i
          ...
          <!--timer running-->
          <span class="x-icon text-danger {{ runtimeCardMyRunningTimer($task->timer_current_status) }}" id="card-task-timer-{{ $task->task_id }}"><i class="mdi mdi-timer"></i></span>
