...
<div class="card-show-form-data" id="card-lead-organisation">
  @if (count($fields ?? []) > 0)
    @foreach ($fields as $field)
      <div class="form-data-row">
        ...
        <span class="x-data-content {{ $field->customfields_datatype }}">{!! customFieldValueDisplay($field->customfields_name, $lead, $field->customfields_datatype) !!}</span>
    ...
    @if (config('visibility.lead_editing_buttons'))
      <div class="form-data-row-buttons">
        <button type="button" class="btn waves-effect waves-light btn-xs btn-info ajax-request"
          data-url="{{ url('leads/content/' . getLeadId($lead)['leadId'] . '/edit-customfields?type=' . getLeadId($lead)['type']) }}" data-loading-class="loading-before-centre"
  ...
  @else
    <div class="x-no-result">
      ...
      <div class="p-t-20">
        <h4>{{ cleanLang(__('lang.you_do_not_have_custom_fields')) }}</h4>
      </div>
