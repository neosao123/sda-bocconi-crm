<div class="card-checklist" id="card-checklist">
  ...
  @if ($lead->permission_edit_lead)
    <div class="x-action">
      <a href="javascript:void(0)" class="js-card-checklist-toggle" id="card-checklist-add-new" data-action-url="{{ url('/leads/' . getLeadId($lead)['leadId'] . '/add-checklist') }}" data-toggle="new"
        data-type="{{ getLeadId($lead)['type'] }}">{{ cleanLang(__('lang.add_a_new_item')) }}</a>
    </div>
  @endif
</div>
