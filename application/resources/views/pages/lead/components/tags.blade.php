<div class="x-section">
  ...
  <div id="card-tags-edit-tags-container" class="hidden">
    <select name="tags" id="card_tags" class="form-control form-control-sm select2-multiple {{ runtimeAllowUserTags() }} select2-hidden-accessible" multiple="multiple" tabindex="-1"
      aria-hidden="true">
    ...
    <div id="card-edit-tags-buttons" class="p-t-10 hidden text-right display-block">
      <button type="button" class="btn waves-effect waves-light btn-xs btn-default" id="card-tags-button-cancel">@lang('lang.close')</button>
      <button type="button" class="btn waves-effect waves-light btn-xs btn-danger ajax-request"
        data-url="{{ url('leads/' . getLeadId($lead)['leadId'] . '/update-tags?type=' . getLeadId($lead)['type']) }}" data-progress-bar="hidden" data-type="form" data-form-id="card-tags-container"
        data-ajax-type="post" id="card-tags-button-save">@lang('lang.save')</button>
    </div>
  </div>
</div>
