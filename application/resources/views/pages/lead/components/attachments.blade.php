<div class="card-attachments" id="card-attachments" data-upload-url="{{ url('/leads/' . getLeadId($lead)['leadId'] . '/attach-files?type=' . getLeadId($lead)['type']) }}">
  ...
  @if ($lead->permission_participate)
    ...
    <div class="hidden" id="card-fileupload-container">
      ...
      <div class="form-group row">
        <label class="col-12 text-left control-label col-form-label required p-l-35">{{ cleanLang(__('lang.tags')) }}</label>
        <div class="col-12 p-l-35">
          <select name="tags" id="tags" class="form-control form-control-sm select2-multiple {{ runtimeAllowUserTags() }} select2-hidden-accessible card-attachment-tags" multiple="multiple"
