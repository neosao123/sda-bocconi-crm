@foreach ($comments as $comment)
  <div class="display-flex flex-row comment-row" id="card_comment_{{ $comment->comment_id }}">
    <div class="p-2 comment-avatar">
      <img src="{{ getUsersAvatar($comment->avatar_directory, $comment->avatar_filename) }}" class="img-circle" alt="{{ $comment->first_name ?? runtimeUnkownUser() }}" width="40">
    </div>
        ...
        <div class="col-sm-6 x-meta text-right">
          ...
          @if ($comment->permission_delete_comment)
            <span class="comment-actions"> |
              <a href="javascript:void(0)" class="js-delete-ux-confirm confirm-action-danger text-danger" data-confirm-title="{{ cleanLang(__('lang.delete_item')) }}"
                data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}" data-ajax-type="DELETE" data-parent-container="card_comment_{{ $comment->comment_id }}" data-progress-bar="hidden"
