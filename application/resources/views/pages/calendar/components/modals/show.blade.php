...
<div class="calendar-show-container" id="calendar-show-container">
    ...
    <div class="row event-details">
        <div class="col-lg-3 event-icon">
            <ion-icon name="calendar-clear-outline"></ion-icon> <span>@lang('lang.date')</span>
        </div>
    ...
    <div class="row event-details">
        <div class="col-lg-3 event-icon">
            <ion-icon name="person-outline"></ion-icon> <span>@lang('lang.added_by')</span>
        </div>
        <div class="col-lg-9 event-info">
        ...
        <div class="col-lg-9 event-info">
            @if ($event['extendedProps']['resource_type'] == 'project')
                <span class="label label-outline-success">
                    <a href="{{ url('/projects/' . $event['extendedProps']['resource_id']) }}">
                        @lang('lang.project')
                    </a>
                </span>
            @endif
            ...
            @if ($event['extendedProps']['resource_type'] == 'task')
                <span class="label label-outline-success">
                    <a href="{{ url('/tasks/v/' . $event['extendedProps']['resource_id']) }}">
                        @lang('lang.task')
                    </a>
                </span>
            @endif
    ...
    @if ($event['extendedProps']['resource_type'] == 'task')
        <div class="row event-details">
            ...
            <div class="col-lg-9 event-info">
                <a href="{{ url('/projects/' . $event['extendedProps']['project_id']) }}">
    ...
    @if ($event['extendedProps']['resource_type'] == 'project')
        <div class="row event-details">
            ...
            <div class="col-lg-9 event-info">
                <span
                    class="label {{ runtimeProjectStatusColors($event['extendedProps']['project_status'], 'label') }}">
                    {{ runtimeLang($event['extendedProps']['project_status']) }}
                </span>
            </div>
        </div>
    @endif
    ...
    <div class="row event-details">
        <div class="col-lg-3 event-icon">
            <ion-icon name="people-outline" class="people"></ion-icon> <span>@lang('lang.users')</span>
        </div>
        <div class="col-lg-9 event-info">
            <!--self-->
    ...
    <div class="text-right p-t-10 p-b-30">
        ...
        @if ($event['extendedProps']['edit_permission'])
            <a type="submit" class="btn btn-rounded-x btn-info waves-effect text-left ajax-request"
                ...
                data-form-id="commonModalBody">@lang('lang.edit')</a>
        @endif
    ...
    <div class="calendar-edit-container hidden" id="calendar-edit-container">
        ...
        <div class="text-right p-t-10 p-b-30">
            ...
            <button type="submit" class="btn btn-rounded-x btn-danger waves-effect text-left ajax-request"
                data-url="{{ url('/calendar/' . $event['id']) }}" data-loading-target="commonModalBody"
