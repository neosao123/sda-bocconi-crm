    ...
    <tr id="contact_{{ $contact->id }}">
        ...
        @if (config('visibility.contacts_col_client'))
            <td class="contacts_col_company" id="contacts_col_company_{{ $contact->id }}">
                <a
                    href="{{ url('/clients') }}/{{ $contact->clientid }}">{{ str_limit($contact->client_company_name, 15) }}</a>
            </td>
        @endif
                ...
                <span class="list-table-action dropdown font-size-inherit">
                    ...
                    @if (config('visibility.action_buttons_edit'))
                        <button type="button" title="{{ cleanLang(__('lang.edit')) }}"
                            ...
                            data-url="{{ urlResource('/contacts/' . $contact->id . '/edit') }}"
                            data-loading-target="commonModalBody"
                            data-modal-title="{{ cleanLang(__('lang.edit_user')) }}"
                            data-action-url="{{ urlResource('/contacts/' . $contact->id . '?ref=list') }}"
